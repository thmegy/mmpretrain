    ...
    if folder_to_idx is not None:
        for folder_name in sorted(list(folder_to_idx.keys())):
            ...
            for d in sorted(list(dirs)):
                dir_path = backend.join_path(_dir, d)
                ...
                for f in sorted(list(files)):
                    if is_valid_file(f):
                        path = backend.join_path(dir_path, f)
                        file_list.append(path)

                if len(file_list) > 0:    
                    item = (file_list, folder_to_idx[folder_name])
                    samples.append(item)
